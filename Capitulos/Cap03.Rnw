...
\begin{definition}
{\em Dado un espacio vectorial $E$ sobre $\m{K}$. Llamaremos espacio dual de $E$ y lo notaremos por $E^*$, al espacio vectorial
\[E^*=\C{L}(E,\m{K})=\{f:E\to \m{K}~/~\text{ $f$ es aplicación lineal.}\}\]

A los vectores $f\in E^*$, los denominaremos formas lineales sobre $\m{K}$.}
\end{definition}
...
\begin{definition}
{\em Sea $E$ un espacio vectorial definido sobre $\m{K}$ y sea $S$ un subespacio de $E$, se define el \emph{aniquilador} de $S$ como el siguiente subespacio vectorial de $E^*$.
